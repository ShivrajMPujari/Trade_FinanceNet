...
PEER0_importer_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/importer.bridgeIt.com/peers/peer0.importer.bridgeIt.com/tls/ca.crt
PEER0_exporter_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/exporter.bridgeIt.com/peers/peer0.exporter.bridgeIt.com/tls/ca.crt
PEER0_custom_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/custom.bridgeIt.com/peers/peer0.custom.bridgeIt.com/tls/ca.crt
PEER0_importerBank_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/importerBank.bridgeIt.com/peers/peer0.importerBank.bridgeIt.com/tls/ca.crt
PEER0_insurance_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/insurance.bridgeIt.com/peers/peer0.insurance.bridgeIt.com/tls/ca.crt
...
instantiateChaincode() {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		set -x
		peer chaincode instantiate -o orderer.bridgeIt.com:7050 -C $CHANNEL_NAME -n tradefinancecc -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["init","a","100","b","200"]}' -P "OR	('ImporterMSP.peer','ExporterMSP.peer','CustomMSP.peer','ImporterBankMSP.peer','InsuranceMSP.peer')" >&log.txt
		res=$?
		set +x
	else
		set -x
		peer chaincode instantiate -o orderer.bridgeIt.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n tradefinancecc -l ${LANGUAGE} -v 1.0 -c '{"Args":["init","a","100","b","200"]}' -P "AND	('ImporterMSP.peer','ExporterMSP.peer')" >&log.txt
		res=$?
		set +x
	fi
	cat log.txt
...
upgradeChaincode() {
	...
	set -x
	peer chaincode upgrade -o orderer.bridgeIt.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n tradefinancecc -v 2.0 -c '{"Args":["init","a","90","b","210"]}' -P "OR ('ImporterMSP.peer','ExporterMSP.peer')"
	res=$?
	set +x
	cat log.txt
...
parsePeerConnectionParameters() {
	...
	while [ "$#" -gt 0 ]; do
		PEER="peer$1.org$2"
		PEERS="$PEERS $PEER"
		PEER_CONN_PARMS="$PEER_CONN_PARMS --peerAddresses $PEER.bridgeIt.com:7051"
...
chaincodeInvoke() {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		set -x
		peer chaincode invoke -o orderer.bridgeIt.com:7050 -C $CHANNEL_NAME -n tradefinancecc -c '{"Args":["invoke","a","10","b","20"]}' >&log.txt
		res=$?
		set +x
	else
		set -x
		peer chaincode invoke -o orderer.bridgeIt.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n tradefinancecc -c '{"Args":["invoke","a","10","b","20"]}' >&log.txt
		res=$?
		set +x
	fi
	cat log.txt
