...
function clearContainers() {
  CONTAINER_IDS=$(docker ps -a | awk '($2 ~ /dev-peer.*.tradefinancecc.*/) {print $1}')
...
function removeUnwantedImages() {
  DOCKER_IMAGE_IDS=$(docker images | awk '($1 ~ /dev-peer.*.tradefinancecc.*/) {print $3}')
...
function upgradeNetwork() {
  ...
  for PEER in peer0.importer.bridgeIt.com peer0.exporter.bridgeIt.com peer0.custom.bridgeIt.com peer0.importerBank.bridgeIt.com peer0.insurance.bridgeIt.com; do
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run -v $PWD:/tmp/trade-finance --rm hyperledger/fabric-tools:$IMAGETAG rm -Rf /tmp/trade-finance/ledgers-backup
...
function generateChannelArtifacts() {
  ...
  configtxgen -profile FiveOrgsChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/ExporterMSPanchors.tx -channelID $CHANNEL_NAME -asOrg ExporterMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  set -x
  configtxgen -profile FiveOrgsChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/CustomMSPanchors.tx -channelID $CHANNEL_NAME -asOrg CustomMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  set -x
  configtxgen -profile FiveOrgsChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/ImporterBankMSPanchors.tx -channelID $CHANNEL_NAME -asOrg ImporterBankMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
